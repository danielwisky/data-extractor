...
public class ShopcarExtractService implements VehicleExtractService {
  ...
  private Vehicle enrichVehicle(final Vehicle vehicle) {
    ...
    try {
      ...
      final Elements detailMotorElements = detailAsDoc.select("li.l2.motor tr td");

      final Elements detailRodaElements = detailAsDoc.select("li.l2.desempenho-rodas tr td");

      vehicle.setSpeed(parseASDouble(detailRodaElements.get(2).text()));
      vehicle.setTime0to100(parseASDouble(detailRodaElements.get(4).text()));

      vehicle.setWeight(extractWeight(detailAsDoc));
      vehicle.setEngine(detailMotorElements.get(2).text());
      vehicle.setMetrics(new ArrayList<>());

      String[] tipoAsSplit = detailMotorElements.get(12).text().replace("n/d", "ND").split("/");
      String[] potenciaAsSplit = detailMotorElements.get(14).text().split("/");
      String[] torqueAsSplit = detailMotorElements.get(18).text().split("/");

      String[] cityAsSplit = detailRodaElements.get(6).text().replace("N/D", "ND").split("/");
      String[] roadAsSplit = detailRodaElements.get(8).text().replace("N/D", "ND").split("/");
      ...
      for (int i = 0; i < tipoAsSplit.length; i++) {
        ...
        try {
          metric.setPower(parseASDouble(StringUtils.trim(potenciaAsSplit[i])));
        } catch (Exception e) {
          metric.setPower(parseASDouble(StringUtils.trim(potenciaAsSplit[0])));
        }

        try {
          metric.setTorque(parseASDouble(StringUtils.trim(torqueAsSplit[i])));
        } catch (Exception e) {
          metric.setTorque(parseASDouble(StringUtils.trim(torqueAsSplit[0])));
        }

        try {
          metric.setCity(parseASDouble(StringUtils.trim(cityAsSplit[i])));
        } catch (Exception e) {
          metric.setCity(parseASDouble(StringUtils.trim(cityAsSplit[0])));
        }

        try {
          metric.setRoad(parseASDouble(StringUtils.trim(roadAsSplit[i])));
        } catch (Exception e) {
          metric.setRoad(parseASDouble(StringUtils.trim(roadAsSplit[0])));
        }
  ...
  private Double parseASDouble(String valueAsString) {
    String value = valueAsString.replaceAll(",", ".").replaceAll("[^0-9.]+", "");
    return !isParsable(value) ? null : Double.parseDouble(value);
  }
}